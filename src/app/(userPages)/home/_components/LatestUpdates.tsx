...
/**
 * LatestUpdates section — responsive design with different card layouts for mobile, tablet, and desktop
 */
  ...
  useEffect(() => {
    ...
    const updateCardsPerView = () => {
      if (window.innerWidth < 640) {
        // Mobile: 1 full card + 0.3 teaser
        setCardsPerView(1.3)
      } else if (window.innerWidth < 1024) {
        // Tablet: 2 full cards + 0.5 teaser
        setCardsPerView(2.5)
      } else {
        // Desktop: 3 full cards + 0.5 teaser
        setCardsPerView(3.5)
      }
    }
    ...
    <section className="w-full bg-white">
      <div className="max-w-[1366px] mx-auto py-8 md:py-12 lg:py-16">
        ...
        <div className="px-4 md:px-8 lg:px-16">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 lg:gap-4 mb-8 lg:mb-10">
            <div className="w-full lg:w-auto">
              <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 leading-tight mb-3 lg:mb-2">
                Latest Updates &<br className="hidden sm:block" />
                <span className="sm:hidden"> </span>Community Events
              </h2>
              <p className="text-gray-600 text-sm md:text-base max-w-xl leading-relaxed">
                Explore our latest news, upcoming events, and important updates as we continue our mission to create
            ...
            <div className="hidden lg:flex flex-col items-end gap-3">
              <Button asChild variant="link" className="text-gray-700 hover:text-black p-0 h-auto font-medium">
              ...
              <div className="flex gap-2">
                <Button
                  size="icon"
                  variant="outline"
                  className={`cursor-pointer w-10 h-10 rounded-full border-[#007466] text-[#007466] ${
                    canPrev ? "hover:bg-[#007466]/10" : "opacity-50 cursor-not-allowed"
                  }`}
                  disabled={!canPrev}
                  onClick={prev}
                >
                ...
                <Button
                  size="icon"
                  variant="outline"
                  className={`cursor-pointer w-10 h-10 rounded-full bg-[#007466] text-white border-[#007466] ${
                    canNext ? "hover:bg-[#01453C]" : "opacity-50 cursor-not-allowed"
                  }`}
                  disabled={!canNext}
                  onClick={next}
                >
            ...
            <div className="flex lg:hidden w-full justify-between items-center">
              ...
              <div className="flex gap-2">
                <Button
                  size="icon"
                  variant="outline"
                  className={`cursor-pointer w-9 h-9 md:w-10 md:h-10 rounded-full border-[#007466] text-[#007466] ${
                    canPrev ? "hover:bg-[#007466]/10" : "opacity-50 cursor-not-allowed"
                  }`}
                  disabled={!canPrev}
                  onClick={prev}
                ...
                <Button
                  size="icon"
                  variant="outline"
                  className={`cursor-pointer w-9 h-9 md:w-10 md:h-10 rounded-full bg-[#007466] text-white border-[#007466] ${
                    canNext ? "hover:bg-[#01453C]" : "opacity-50 cursor-not-allowed"
                  }`}
                  disabled={!canNext}
                  onClick={next}
          ...
          >
            {updates.map((u) => (
              <div key={u.id} style={{ width: `${CARD_WIDTH_PERCENT}%` }} className="flex-shrink-0">
                <Card className="h-full rounded-lg md:rounded-xl border border-gray-50 overflow-hidden group hover:shadow-md transition-shadow duration-200">
                  <div className="relative h-40 sm:h-44 md:h-48 lg:h-52 w-full p-2 md:p-3 lg:p-4">
                    <div className="relative w-full h-full rounded-md md:rounded-lg overflow-hidden">
                      <Image
                  ...
                  <CardContent className="px-3 md:px-4 pb-4 md:pb-6">
                    <p className="text-xs md:text-sm text-gray-500 mb-2">
                      {u.author} • {u.date}
                    </p>
                    <h3 className="cursor-pointer hover:text-[#f25f4a] transition-colors duration-200 text-sm sm:text-base md:text-lg font-semibold text-gray-900 group-hover:text-gray-700 mb-2 line-clamp-2">
                      {u.title}
                    </h3>
          ...
          {Array.from({ length: maxIndex + 1 }).map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-colors duration-200 ${
                current === index ? "bg-[#007466]" : "bg-gray-300"
              }`}
            />
          ))}
        </div>
